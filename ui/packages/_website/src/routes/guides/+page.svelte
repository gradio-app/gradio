    ...
    function search() {
        for (const category in data.guides_by_category) {
            for (const guide in data.guides_by_category[category].guides) {
                ...
                data.guides_by_category[category].guides[guide].hidden = !(g.pretty_name.toLowerCase().includes(search_query.toLowerCase()) || g.content.toLowerCase().includes(search_query.toLowerCase()))
            }
        }
    }
</script>
