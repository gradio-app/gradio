...
Gradio also supports **local state**, where data persists in the browser's localStorage even after the page is refreshed or closed. This is useful for storing user preferences, settings, or other data that should persist across sessions. To use local state:

1. Create a `gr.LocalState()` object. You can optionally provide an initial value and a key to identify the data in localStorage.
2. Use it like a regular State component in event listeners as inputs and outputs.

Here's a simple example that saves a user's notes across sessions:
