...
class EndpointV3Compatibility:
    ...
    def make_predict(self, helper: Communicator | None = None) -> Callable:
        def _predict(*data) -> tuple:
            ...
            try:
                ...
                raise ValueError(f"HTTP error occurred: {e.response.text} (Status Code: {e.response.status_code})") from e
    ...
    def _upload(
        ...
        if r.status_code != 200:
            raise ValueError(f"Upload failed with status code: {r.status_code}. Response: {r.text}")
        else:
            uploaded = []
            result = r.json()
            for i, fs in enumerate(file_paths):
