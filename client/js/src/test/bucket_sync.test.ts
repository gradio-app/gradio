...
function make_run(id: string, started_at: string, completed_at?: string): StoredRun {
	return {
		id,
		endpoint: "/predict",
		api_name: "predict",
...
describe("merge_runs", () => {
	it("dedupes by id, keeping the fresher run", () => {
		const local = [make_run("a", "2026-01-01T00:00:00Z", "2026-01-01T00:00:05Z")];
		const remote = [make_run("a", "2026-01-01T00:00:00Z", "2026-01-01T00:00:10Z")];
	...
	it("sorts newest-first by started_at", () => {
		const merged = merge_runs(
			...
			[make_run("b", "2026-01-02T00:00:00Z"), make_run("c", "2026-01-03T00:00:00Z")]
		);
