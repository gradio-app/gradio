...
describe('filesize', () => {
	...
	describe('SPECS constant', () => {
		...
		it('should have correct unit arrays', () => {
			expect(SPECS.si.unit).toEqual(['b', 'kb', 'Mb', 'Gb', 'Tb', 'Pb', 'Eb', 'Zb', 'Yb']);
			expect(SPECS.iec.unit).toEqual(['b', 'Kib', 'Mib', 'Gib', 'Tib', 'Pib', 'Eib', 'Zib', 'Yib']);
			expect(SPECS.jedec.unit).toEqual(['b', 'Kb', 'Mb', 'Gb', 'Tb', 'Pb', 'Eb', 'Zb', 'Yb']);
		});
	});
});
