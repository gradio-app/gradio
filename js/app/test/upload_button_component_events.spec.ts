...
test("UploadButton properly dispatches load event  and click event for the single file case.", async ({
    ...
    await expect(page.getByLabel('# Click Upload Single File Output')).toHaveValue('1');
...
test("UploadButton properly dispatches load event  and click event for the multiple file case.", async ({
    ...
    const uploader = await page.getByTestId("Upload Multiple Files-upload-button");
	await Promise.all([
		uploader.setInputFiles(["./test/files/face.obj", "./test/files/cheetah1.jpg"]),
		page.waitForResponse("**/upload?*?*")
	]);

    await expect(page.getByLabel('# Load Upload Multiple Files')).toHaveValue('1');
    await expect(page.getByLabel('# Click Upload Multiple Files Output')).toHaveValue('1');
