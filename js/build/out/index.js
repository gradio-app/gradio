...
function generate_cdn_entry({ version, cdn_base }) {
  return {
    ...
    writeBundle(config, bundle) {
      ...
      const transformed_html =
        bundle["index.html"].source.substring(0, script?.range[0]) +
        `<script type="module" crossorigin src="${cdn_base}/${version}/gradio.js"></script>` +
        bundle["index.html"].source.substring(script?.range[1], source.length);
