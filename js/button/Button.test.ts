...
test.todo(
	"VISUAL: variant='primary' renders with primary button styling — needs Playwright visual regression"
);

test.todo(
	"VISUAL: variant='secondary' renders with secondary button styling — needs Playwright visual regression"
);

test.todo(
	"VISUAL: variant='stop' renders with stop/cancel button styling — needs Playwright visual regression"
);

test.todo(
	"VISUAL: variant='huggingface' renders with HuggingFace branding — needs Playwright visual regression"
);

test.todo(
	"VISUAL: size='sm' renders with small button sizing — needs Playwright visual regression"
);

test.todo(
	"VISUAL: size='md' renders with medium button sizing — needs Playwright visual regression"
);

test.todo(
	"VISUAL: size='lg' renders with large button sizing — needs Playwright visual regression"
);

test.todo(
	"VISUAL: icon prop renders an icon image alongside text — needs Playwright visual regression"
);

test.todo(
	"VISUAL: scale prop affects flex-grow layout — needs Playwright visual regression"
);

test.todo(
	"VISUAL: min_width prop constrains the button's minimum width — needs Playwright visual regression"
);

test.todo(
	"VISUAL: disabled button has reduced opacity and grayscale filter — needs Playwright visual regression"
);
