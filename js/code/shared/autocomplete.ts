...
export function create_pyodide_autocomplete(): ((context: CompletionContext) => Promise<CompletionResult | null>) | null {
	...
	return async function pyodide_autocomplete(context: CompletionContext): Promise<CompletionResult | null> {
		try {
			...
			return {
				...
				options: completions.map((completion) => ({
					...
					boost: (
						(completion.label.startsWith("_") ? -1 : 0) // Items starting with "_" are private attributes and should be sorted last.
					),
				})),
			};
		} catch (e) {
