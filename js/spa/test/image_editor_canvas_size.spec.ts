...
test.fixme(
	"Image Editor canvas matches background image size if fixed_canvas=False",
	async ({ page }) => {
...
test.fixme(
	"Image Editor 300 x 300 canvas resizes to match uploaded image",
	async ({ page }) => {
		const [fileChooser] = await Promise.all([
			page.waitForEvent("filechooser"),
...
test.fixme(
	"Image Editor 300 x 300 canvas maintains size while being drawn upon",
	async ({ page }) => {
...
test.fixme(
	"Image Editor reshapes image to fit fixed 500 x 500 canvas",
	async ({ page }) => {
		const [fileChooser] = await Promise.all([
			page.waitForEvent("filechooser"),
