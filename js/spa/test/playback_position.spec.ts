...
test("Video playback position is retrieved correctly and updates as video plays.", async ({
	...
	await page.getByRole("button", { name: "Get Video Playback Position" }).click();
...
test("Audio playback position is retrieved correctly and updates as audio plays.", async ({
	...
	await page.getByTestId("waveform-Audio").getByLabel("Play", { exact: true }).click();
	await page.waitForTimeout(2000);

	await page.getByRole("button", { name: "Get Audio Playback Position" }).click();
	await page.waitForTimeout(500);
