...
test.fixme("Search bar filters dataframe correctly.", async ({ page }) => {
...
test.fixme(
	"Column selection adds columns to the dataframe.",
	async ({ page }) => {
...
test.fixme(
	"Column selection removes columns to the dataframe.",
	async ({ page }) => {
		await page.waitForSelector("#leaderboard-table");
...
test.fixme(
	"Model Types Checkbox filters models from the table",
	async ({ page }) => {
		await expect(
