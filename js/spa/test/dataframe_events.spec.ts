...
test("Dataframe can be cleared and updated indirectly", async ({ page }) => {
	...
	const df_block = page.locator("#dataframe_tall");
	...
	expect(trimmed_headers).toEqual([
		"Model     ⋮",
		"% Correct (LeetCode Hard)     ⋮",
		"Is Open Source     ⋮"
	]);
});
...
test("Dataframe keyboard operations work as expected", async ({ page }) => {
	...
	// test delete key
	const cell = get_cell(df, 0, 0);
	await cell.click();
	// await page.waitForTimeout(100);
	await cell.press("Escape");
	await cell.press("Delete");

	await expect(get_cell(df, 0, 0)).toHaveText("    ⋮");
	...
	expect(clipboard_value).toContain("test_copy_value");
});
...
test("Dataframe displays custom display values with medal icons correctly", async ({
	...
	await expect(get_cell(tall_df, 0, 0)).toContainText("🥇");
	await expect(get_cell(tall_df, 1, 0)).toContainText("🥈");
	await expect(get_cell(tall_df, 2, 0)).toContainText("🥉");

	// no medals for 4th position
	const fourth_cell = get_cell(tall_df, 3, 0);
	await expect(fourth_cell).not.toContainText("🥇");
	await expect(fourth_cell).not.toContainText("🥈");
	await expect(fourth_cell).not.toContainText("🥉");

	// verify medals don't appear in other columns
	await expect(get_cell(tall_df, 0, 1)).not.toContainText("🥇");
});
...
test("Dataframe static columns cannot be cleared with Delete key", async ({
	...
	await static_cell.press("Escape");
	await static_cell.press("Delete");

	expect(initial_static_cell_value).toBe("0");

	const editable_cell = get_cell(df, 0, 1);
	await editable_cell.click();
	await editable_cell.press("Escape");
	await editable_cell.press("Backspace");

	await expect(editable_cell).toHaveText("    ⋮");
});
...
test("Dataframe keyboard events allow newlines", async ({ page }) => {
	...
	await expect(get_cell(df, 0, 0)).toContainText(" 42\ndon't panic");
});
