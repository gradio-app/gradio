...
export async function GET() {
	const gradio_doc_paths = import.meta.glob("/src/lib/templates/gradio/**/*.svx");
	const gradio_doc_pages = await Promise.all(Object.entries(gradio_doc_paths)
	...
	const client_doc_paths = import.meta.glob("/src/lib/templates/python-client/**/*.svx");
	const client_doc_pages = await Promise.all(Object.entries(client_doc_paths)
		.map(async ([path, content]) => {
			content = await content();
			content = content.default.render().html;
			let match = content.match(/<h1[^>]*>(.*?)<\/h1>/i)
	...
	const js_pages = await Promise.all(Object.entries(js_components)
	...
	let all_pages = gradio_doc_pages.concat(client_doc_pages).concat(guide_pages).concat([js_client_page]).concat(js_pages)
