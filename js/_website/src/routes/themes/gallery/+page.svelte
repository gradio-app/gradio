<script lang="ts">
	...
	import { BUILTIN_THEMES } from "./utils";
	...
	async function handle_search(e: KeyboardEvent): Promise<void> {
		await tick();
	...
	onMount(() => {
		themes = filter_themes("", "all");

		if (browser) {
			const id = $page.url.searchParams.get("id");
			if (id) {
				selected_theme = themes.find((t) => t.id === id) ?? null;
			}
		}
	...
	<div class="flex flex-wrap items-center justify-between gap-3 mb-4">
		<div class="flex items-center gap-2">
			...
			<div class="flex gap-1">
				...
				>
					All <span class="opacity-70">{BUILTIN_THEMES.length}</span>
				</button>
				<button
		...
		<div class="flex items-center gap-3">
			<input
				type="text"
				class="w-48 border border-gray-200 dark:border-gray-700 dark:bg-neutral-800 dark:text-gray-200 px-3 py-1.5 rounded-lg outline-none text-sm focus:border-orange-500 focus:ring-0"
				placeholder="Search..."
				autocomplete="off"
