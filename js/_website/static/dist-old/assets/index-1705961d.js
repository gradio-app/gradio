import { C as R, E as m, L as C, a as u } from "./index-4910755d.js";
import {
	s as z,
	t as e,
	y as n,
...
const y = 301,
	...
	ZO = new m(
		(O, Q) => {
			let { next: i } = O;
			(i == _ || i == -1 || Q.context) && Q.canShift(d) && O.acceptToken(d);
		},
	...
	XO = new m(
		(O, Q) => {
			let { next: i } = O;
			if ((i == PO || i == nO) && (O.advance(), i == O.next)) {
				O.advance();
				let a = !Q.context && Q.canShift(j);
				O.acceptToken(a ? j : QO);
			}
		},
		{ contextual: !0 }
	),
	cO = z({
		...
		"let var const function class extends": e.definitionKeyword,
		...
		"FunctionDeclaration/VariableDefinition": e.function(
	...
	YO = C.deserialize({
		...
		tokenizers: [
			...
			new u("j~RQYZXz{^~^O'p~~aP!P!Qd~iO'q~~", 25, 307)
		],
		topRules: {
			Script: [0, 5],
		...
		specialized: [
			{ term: 311, get: (O) => sO[O] || -1 },
			{ term: 327, get: (O) => pO[O] || -1 },
			{ term: 67, get: (O) => gO[O] || -1 }
		],
		tokenPrec: 13238
	}),
	bO = [
		n("function ${name}(${params}) {\n	${}\n}", {
			label: "function",
			detail: "definition",
			type: "keyword"
		}),
		n("for (let ${index} = 0; ${index} < ${bound}; ${index}++) {\n	${}\n}", {
			label: "for",
			detail: "loop",
			type: "keyword"
		}),
		n("for (let ${name} of ${collection}) {\n	${}\n}", {
			label: "for",
			detail: "of loop",
			type: "keyword"
		}),
	...
	constructor(\${params}) {
		...
		n('import {${names}} from "${module}"\n${}', {
			label: "import",
			detail: "named",
			type: "keyword"
		}),
		n('import ${name} from "${module}"\n${}', {
			label: "import",
			detail: "default",
			type: "keyword"
		})
...
const hO = ["FunctionDeclaration"],
	mO = {
		FunctionDeclaration: c("function"),
		ClassDeclaration: c("class"),
		ClassExpression: () => !0,
		EnumDeclaration: c("constant"),
		TypeAliasDeclaration: c("type"),
...
function q(O, Q) {
	...
	let a = [],
		$ = !0;
	function t(r, S) {
		let o = O.sliceString(r.from, r.to);
		a.push({ label: o, type: S });
	}
	return (
		Q.cursor(M.IncludeAnonymous).iterate((r) => {
			if ($) $ = !1;
			else if (r.name) {
				let S = mO[r.name];
				if ((S && S(r, t)) || G.has(r.name)) return !1;
			} else if (r.to - r.from > 8192) {
				for (let S of q(O, r.node)) a.push(S);
				return !1;
			}
		}),
		v.set(Q, a),
...
function WO(O) {
	let Q = W(O.state).resolveInner(O.pos, -1);
	if (U.indexOf(Q.name) > -1) return null;
	let i =
		Q.name == "VariableName" ||
		(Q.to - Q.from < 20 && g.test(O.state.sliceDoc(Q.from, Q.to)));
	if (!i && !O.explicit) return null;
	let a = [];
	for (let $ = Q; $; $ = $.parent)
		G.has($.name) && (a = a.concat(q(O.state.doc, $)));
	return { options: a, from: i ? Q.from : O.pos, validFor: g };
}
function h(O, Q, i) {
	var a;
	let $ = [];
	for (;;) {
		...
		if (t?.name == "VariableName")
			return $.push(O(t)), { path: $.reverse(), name: i };
		if (
			t?.name == "MemberExpression" &&
			((a = r = t.lastChild) === null || a === void 0 ? void 0 : a.name) ==
				"PropertyName"
		)
			$.push(O(r)), (Q = t);
		else return null;
...
function UO(O) {
	let Q = (a) => O.state.doc.sliceString(a.from, a.to),
		i = W(O.state).resolveInner(O.pos, -1);
	return i.name == "PropertyName"
		? h(Q, i.parent, Q(i))
		: U.indexOf(i.name) > -1
		? null
		: i.name == "VariableName" || (i.to - i.from < 20 && g.test(Q(i)))
		? { path: [], name: Q(i) }
		: (i.name == "." || i.name == "?.") && i.parent.name == "MemberExpression"
		? h(Q, i.parent, "")
		: i.name == "MemberExpression"
		? h(Q, i, "")
		: O.explicit
		? { path: [], name: "" }
		: null;
...
function AO(O) {
	...
	return (i) => {
		...
		if (!a) return null;
		let $ = O;
		for (let r of a.path) if ((($ = $[r]), !$)) return null;
		let t = Q.get($);
		return (
			t || Q.set($, (t = fO($, !a.path.length))),
			{ from: i.pos - a.name.length, options: t, validFor: g }
		);
	};
}
const X = I.define({
	...
	T = {
		...
		facet: F({ commentTokens: { block: { open: "{/*", close: "*/}" } } })
	},
	uO = X.configure({ dialect: "ts" }, "typescript"),
	yO = X.configure({
		dialect: "jsx",
		props: [k.add((O) => (O.isTop ? [T] : void 0))]
	}),
	jO = X.configure(
		{ dialect: "jsx ts", props: [k.add((O) => (O.isTop ? [T] : void 0))] },
		"typescript"
	),
	dO =
		"break case const continue default delete export extends false finally in instanceof let new return static super switch this throw true typeof var yield"
			.split(" ")
			.map((O) => ({ label: O, type: "keyword" }));
function JO(O = {}) {
	let Q = O.jsx ? (O.typescript ? jO : yO) : O.typescript ? uO : X;
	return new D(Q, [
		X.data.of({ autocomplete: B(U, H(bO.concat(dO))) }),
		X.data.of({ autocomplete: WO }),
		O.jsx ? wO : []
	]);
}
function xO(O) {
...
function w(O, Q, i = O.length) {
	for (let a = Q?.firstChild; a; a = a.nextSibling)
		if (
			...
			return O.sliceString(a.from, Math.min(a.to, i));
	return "";
}
const vO =
		typeof navigator == "object" && /Android\b/.test(navigator.userAgent),
	wO = K.inputHandler.of((O, Q, i, a) => {
		if (
			(vO ? O.composing : O.compositionStarted) ||
			O.state.readOnly ||
			Q != i ||
			(a != ">" && a != "/") ||
			!X.isActiveAt(O.state, Q, -1)
		)
			return !1;
		let { state: $ } = O,
			t = $.changeByRange((r) => {
				var S, o;
				let { head: P } = r,
					Z = W($).resolveInner(P, -1),
					s;
				if (
					(Z.name == "JSXStartTag" && (Z = Z.parent),
					a == ">" && Z.name == "JSXFragmentTag")
				)
					return {
						range: b.cursor(P + 1),
						changes: { from: P, insert: "></>" }
					};
				if (a == "/" && Z.name == "JSXFragmentTag") {
					let l = Z.parent,
						p = l?.parent;
					if (
						l.from == P - 1 &&
						((S = p.lastChild) === null || S === void 0 ? void 0 : S.name) !=
							"JSXEndTag" &&
						(s = w($.doc, p?.firstChild, P))
					) {
						let f = `/${s}>`;
						return {
							range: b.cursor(P + f.length),
							changes: { from: P, insert: f }
						};
					}
				} else if (a == ">") {
					let l = xO(Z);
					if (
						l &&
						((o = l.lastChild) === null || o === void 0 ? void 0 : o.name) !=
							"JSXEndTag" &&
						$.sliceDoc(P, P + 2) != "</" &&
						(s = w($.doc, l, P))
					)
		...
		return t.changes.empty
			? !1
			: (O.dispatch(t, { userEvent: "input.type", scrollIntoView: !0 }), !0);
	});
function LO(O, Q) {
	return (
		Q ||
			...
			O.getRules().forEach((i, a) => {
				i.meta.docs.recommended && (Q.rules[a] = 2);
			})),
		(i) => {
			let { state: a } = i,
				$ = [];
			for (let { from: t, to: r } of X.findRegions(a)) {
				let S = a.doc.lineAt(t),
					o = { line: S.number - 1, col: t - S.from, pos: t };
				for (let P of O.verify(a.sliceDoc(t, r), Q)) $.push(VO(P, a.doc, o));
			}
			return $;
		}
	);
...
function V(O, Q, i, a) {
	return i.line(O + a.line).from + Q + (O == 1 ? a.col - 1 : -1);
}
function VO(O, Q, i) {
	let a = V(O.line, O.column, Q, i),
		$ = {
			from: a,
			to:
				O.endLine != null && O.endColumn != 1
					? V(O.endLine, O.endColumn, Q, i)
					: a,
			message: O.message,
		...
		$.actions = [
			{
				...
				apply(P, Z) {
					P.dispatch({
						changes: { from: Z + S, to: Z + o, insert: r },
						scrollIntoView: !0
					});
				}
			}
...
export {
	...
	LO as esLint,
	JO as javascript,
	X as javascriptLanguage,
	yO as jsxLanguage,
	WO as localCompletionSource,
	AO as scopeCompletionSource,
	bO as snippets,
	jO as tsxLanguage,
	uO as typescriptLanguage
};
//# sourceMappingURL=index-1705961d.js.map
