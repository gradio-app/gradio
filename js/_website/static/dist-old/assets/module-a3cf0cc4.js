const w = (t) => (n) => {
	...
	O = (t, n) => (e) => {
		...
		if (!e.has(s)) return t(e, s);
		if (e.size < g) {
			for (; e.has(s); ) s = Math.floor(Math.random() * _);
			return t(e, s);
		}
		if (e.size > f)
			throw new Error(
		...
		for (; e.has(s); ) s = Math.floor(Math.random() * f);
		return t(e, s);
	},
	M = new WeakMap(),
	m = N(M),
		...
		return (e) => {
			...
			e.addEventListener("message", ({ data: o }) => {
				...
				if (a !== null && r.has(a)) {
					...
					r.delete(a),
						o.error === void 0 ? c(o.result) : u(new Error(o.error.message));
				}
			}),
				R(e) && e.start();
			const s = (o, a = null, u = []) =>
					new Promise((c, l) => {
						const d = h(r);
						r.set(d, { reject: l, resolve: c }),
							a === null
								? e.postMessage({ id: d, method: o }, u)
								: e.postMessage({ id: d, method: o, params: a }, u);
					}),
				T = (o, a, u = []) => {
