...
class P {
	constructor(t, e, s, i, h, r, n, a, l, f = 0, u) {
		(this.p = t),
			(this.stack = e),
			(this.state = s),
			(this.reducePos = i),
	...
	reduce(t) {
		...
		let n = this.stack.length - (s - 1) * 3 - (t & 262144 ? 6 : 0),
			a = n ? this.stack[n - 2] : this.p.ranges[0].from,
			l = this.reducePos - a;
		l >= 2e3 &&
			!(
		...
		if (i < h.minRepeatTerm || t & 131072) {
			...
			this.storeNode(i, a, c, u + 4, !0);
		}
		if (t & 262144) this.state = this.stack[n];
		else {
	...
	shift(t, e, s) {
		...
		if (t & 131072) this.pushState(t & 65535, this.pos);
			...
			let h = t,
				{ parser: r } = this.p;
			(s > this.pos || e <= r.maxNode) &&
				((this.pos = s), r.stateFlag(h, 1) || (this.reducePos = s)),
				this.pushState(h, i),
				this.shiftContext(e, i),
				e <= r.maxNode && this.buffer.push(e, i, s, 4);
		}
	}
	apply(t, e, s) {
	...
	recoverByInsert(t) {
		...
		if (e.length > 4 << 1 || this.stack.length >= 120) {
			...
			if (this.stack.length < 120)
				for (let h = 0; i.length < 4 << 1 && h < e.length; h += 2) {
					let r = e[h + 1];
					i.some((n, a) => a & 1 && n == r) || i.push(e[h], r);
				}
			e = i;
		}
		let s = [];
	...
	updateContext(t) {
		if (t != this.curContext.context) {
			let e = new y(this.curContext.tracker, t);
...
var N;
(function (o) {
	(o[(o.Insert = 200)] = "Insert"),
		(o[(o.Delete = 190)] = "Delete"),
		(o[(o.Reduce = 100)] = "Reduce"),
		(o[(o.MaxNext = 4)] = "MaxNext"),
		(o[(o.MaxInsertStackDepth = 300)] = "MaxInsertStackDepth"),
		(o[(o.DampenInsertStackDepth = 120)] = "DampenInsertStackDepth"),
		(o[(o.MinBigReduction = 2e3)] = "MinBigReduction");
})(N || (N = {}));
class W {
	constructor(t) {
		(this.start = t),
...
class C {
	...
	static create(t, e = t.bufferBase + t.buffer.length) {
		return new C(t, e, e - t.bufferBase);
	}
	maybeNext() {
	...
	fork() {
		return new C(this.stack, this.pos, this.index);
	}
}
function x(o, t = Uint16Array) {
	if (typeof o != "string") return o;
	let e = null;
	for (let s = 0, i = 0; s < o.length; ) {
		let h = 0;
		for (;;) {
			let r = o.charCodeAt(s++),
				n = !1;
			if (r == 126) {
				h = 65535;
				break;
...
class J {
	...
	token(t, e) {
		...
		for (
			;
			(i = t.pos),
				F(this.data, t, e, 0, this.data, this.precTable),
				!(t.token.value > -1);
		...
		i > s && (t.reset(s, t.token), t.acceptToken(this.elseToken, i - s));
	}
}
J.prototype.contextual = m.prototype.fallback = m.prototype.extend = !1;
class tt {
	constructor(t, e = {}) {
		(this.token = t),
...
function F(o, t, e, s, i, h) {
	let r = 0,
		n = 1 << s,
		{ dialect: a } = e.p.parser;
	t: for (; n & o[r]; ) {
		let l = o[r + 1];
		for (let p = r + 3; p < l; p += 2)
			if ((o[p + 1] & n) > 0) {
				let d = o[p];
				if (
					a.allows(d) &&
...
function K(o, t, e, s) {
	let i = I(e, s, t);
	return i < 0 || I(e, s, o) < i;
}
const g = typeof process < "u" && process.env && /\bparse\b/.test({}.LOG);
let A = null;
var z;
(function (o) {
...
function B(o, t, e) {
	let s = o.cursor(H.IncludeAnonymous);
	for (s.moveTo(t); ; )
			...
			for (;;) {
				if ((e < 0 ? s.to < t : s.from > t) && !s.type.isError)
					return e < 0
						...
						: Math.min(o.length, Math.max(s.from + 1, t + 25));
		...
		if (t) {
			for (
				this.safeFrom = t.openStart
					? B(t.tree, t.from + t.offset, 1) - t.offset
					: t.from,
					this.safeTo = t.openEnd
						? B(t.tree, t.to + t.offset, -1) - t.offset
						: t.to;
				this.trees.length;
...
class V {
	constructor(t, e) {
		(this.stream = e),
			...
			(this.mainToken = null),
			(this.actions = []),
			(this.tokens = t.tokenizers.map((s) => new S()));
	}
	getActions(t) {
		let e = 0,
			s = null,
		...
		for (let l = 0; l < h.length; l++) {
			...
			if (
				!(s && !f.fallback) &&
				((f.contextual || u.start != t.pos || u.mask != r || u.context != n) &&
					(this.updateCachedToken(u, f, t), (u.mask = r), (u.context = n)),
				u.lookAhead > u.end + 25 && (a = Math.max(u.lookAhead, a)),
				u.value != 0)
			) {
				let c = e;
				if (
					(u.extended > -1 && (e = this.addActions(t, u.extended, u.end, e)),
					(e = this.addActions(t, u.value, u.end, e)),
					!f.extend && ((s = u), e > c))
				)
					break;
			}
	...
	addActions(t, e, s, i) {
		let { state: h } = t,
			...
			{ data: n } = r;
		for (let a = 0; a < 2; a++)
			for (let l = r.stateSlot(h, a ? 2 : 1); ; l += 3) {
				if (n[l] == 65535)
					if (n[l + 1] == 1) l = k(n, l + 2);
					else {
						i == 0 &&
							n[l + 1] == 2 &&
							(i = this.putAction(k(n, l + 2), e, s, i));
						break;
					}
				n[l] == e && (i = this.putAction(k(n, l + 1), e, s, i));
			}
		return i;
	}
}
var M;
(function (o) {
	(o[(o.Distance = 5)] = "Distance"),
		(o[(o.MaxRemainingPerStep = 3)] = "MaxRemainingPerStep"),
		(o[(o.MinBufferLengthPrune = 500)] = "MinBufferLengthPrune"),
		(o[(o.ForceReduceLimit = 10)] = "ForceReduceLimit"),
		(o[(o.CutDepth = 15e3)] = "CutDepth"),
		(o[(o.CutTo = 9e3)] = "CutTo"),
		(o[(o.MaxLeftAssociativeReductionCount = 300)] =
			"MaxLeftAssociativeReductionCount"),
		(o[(o.MaxStackCount = 12)] = "MaxStackCount");
})(M || (M = {}));
class X {
	constructor(t, e, s, i) {
		(this.parser = t),
			...
			(this.stream = new q(e, i)),
			(this.tokens = new V(t, this.stream)),
			(this.topTerm = t.top[1]);
		let { from: h } = i[0];
		(this.stacks = [P.start(this, t.top[0], h)]),
			(this.fragments =
	...
	advance() {
		...
		if (this.recovering) {
			...
			if (s.length > r)
				for (s.sort((n, a) => a.score - n.score); s.length > r; ) s.pop();
			...
			t: for (let r = 0; r < s.length - 1; r++) {
				...
				for (let a = r + 1; a < s.length; a++) {
					...
					if (
						n.sameState(l) ||
						(n.buffer.length > 500 && l.buffer.length > 500)
					)
						if ((n.score - l.score || n.buffer.length - l.buffer.length) > 0)
							s.splice(a--, 1);
						else {
							s.splice(r--, 1);
							continue t;
	...
	advanceStack(t, e, s) {
		...
		if (this.fragments) {
			let l = t.curContext && t.curContext.tracker.strict,
				f = l ? t.curContext.hash : 0;
			for (let u = this.fragments.nodeAt(i); u; ) {
				let c =
					this.parser.nodeSet.types[u.type.id] == u.type
						? h.getGoto(t.state, u.type.id)
						: -1;
				if (c > -1 && u.length && (!l || (u.prop(w.contextHash) || 0) == f))
					return (
						t.useNode(u, c),
						g &&
							console.log(
								r + this.stackID(t) + ` (via reuse of ${h.getName(u.type.id)})`
							),
						!0
					);
				if (!(u instanceof b) || u.children.length == 0 || u.positions[0] > 0)
					break;
				let p = u.children[0];
				if (p instanceof b && u.positions[0] == 0) u = p;
				else break;
			}
		}
		...
		if (n > 0)
			return (
				...
				g &&
					console.log(
						r + this.stackID(t) + ` (via always-reduce ${h.getName(n & 65535)})`
					),
				!0
			);
				...
				g &&
					console.log(
						r +
							this.stackID(d) +
							` (via ${
								f & 65536 ? `reduce of ${h.getName(f & 65535)}` : "shift"
							} for ${h.getName(u)} @ ${i}${d == t ? "" : ", split"})`
					),
				p)
			)
				return !0;
		...
		for (let r = 0; r < t.length; r++) {
			...
			if (
				...
				(h ||
					((h = !0),
					n.restart(),
					g && console.log(f + this.stackID(n) + " (restarted)"),
			...
			for (
				...
				u.forceReduce() &&
				p < 10 &&
				(g && console.log(c + this.stackID(u) + " (via force-reduce)"),
				!this.advanceFully(u, s));
				p++
			)
				g && (c = this.stackID(u) + " -> ");
			for (let p of n.recoverByInsert(a))
				g && console.log(f + this.stackID(p) + " (via recover-insert)"),
					this.advanceFully(p, s);
			this.stream.end > n.pos
				? (l == n.pos && (l++, (a = 0)),
				  n.recoverByDelete(a, l),
				  g &&
						console.log(
							f +
								this.stackID(n) +
	...
	stackID(t) {
		let e = (A || (A = new WeakMap())).get(t);
		return e || A.set(t, (e = String.fromCodePoint(this.nextStackID++))), e + t;
	}
}
function R(o, t) {
...
class et {
	constructor(t) {
		(this.start = t.start),
			(this.shift = t.shift || T),
			(this.reduce = t.reduce || T),
			(this.reuse = t.reuse || T),
			(this.hash = t.hash || (() => 0)),
...
class v extends j {
	constructor(t) {
		...
		function h(n, a, l) {
			i[n].push([a, a.deserialize(String(l))]);
		}
		if (t.nodeProps)
	...
	configure(t) {
		let e = Object.assign(Object.create(v.prototype), this);
...
function O(o) {
	if (o.external) {
		let t = o.extend ? 1 : 0;
		return (e, s) => (o.external(e, s) << 1) | t;
	}
	return o.get;
}
export { et as C, tt as E, v as L, J as a };
//# sourceMappingURL=index-4910755d.js.map
