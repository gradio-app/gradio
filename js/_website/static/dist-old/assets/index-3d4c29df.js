import { E as u, L as v } from "./index-4910755d.js";
import {
	s as k,
	t,
	h as S,
...
const B = new u((e, o) => {
		for (let r = !1, a = 0, O = 0; ; O++) {
			let { next: l } = e;
			if (p(l) || l == c || l == X || (r && I(l)))
				!r && (l != c || O > 0) && (r = !0),
					a === O && l == c && a++,
					e.advance();
			else {
				r && e.acceptToken(l == N ? C : a == 2 && o.canShift(f) ? f : Z);
				break;
			}
		}
	}),
	...
	F = new u((e) => {
		if (!$.includes(e.peek(-1))) {
			let { next: o } = e;
			if ((o == D && (e.advance(), e.acceptToken(g)), p(o))) {
				do e.advance();
				while (p(e.next));
				e.acceptToken(g);
			}
	...
	H = { __proto__: null, not: 128, only: 128, from: 158, to: 160 },
	M = v.deserialize({
		version: 14,
		states:
	...
	if (!Q && typeof document == "object" && document.body) {
		...
		for (let a in e)
			a != "cssText" &&
				...
				typeof e[a] == "string" &&
				(/[A-Z]/.test(a) &&
					(a = a.replace(/[A-Z]/g, (O) => "-" + O.toLowerCase())),
				r.has(a) || (o.push(a), r.add(a)));
		Q = o.sort().map((a) => ({ type: "property", label: a }));
	}
	return Q || [];
}
const h = [
...
function Oe(e, o) {
	...
	if (
		...
		return !1;
	let a = (r = e.parent) === null || r === void 0 ? void 0 : r.firstChild;
	return a?.name != "Callee" ? !1 : o.sliceString(a.from, a.to) == "var";
}
const y = new V(),
	te = ["Declaration"];
function W(e, o) {
	if (o.to - o.from > 4096) {
		...
		let a = [],
			O = new Set(),
			l = o.cursor(T.IncludeAnonymous);
		if (l.firstChild())
			do
				for (let i of W(e, l.node))
					O.has(i.label) || (O.add(i.label), a.push(i));
			while (l.nextSibling());
		return y.set(o, a), a;
	} else {
		let r = [],
		...
		return (
			o.cursor().iterate((O) => {
				...
				if (
					O.name == "VariableName" &&
					O.matchContext(te) &&
					((l = O.node.nextSibling) === null || l === void 0
						? void 0
						: l.name) == ":"
				) {
					let i = e.sliceString(O.from, O.to);
					a.has(i) || (a.add(i), r.push({ label: i, type: "variable" }));
				}
			}),
			r
		);
...
const oe = (e) => {
		...
		if (
			O.name == "PropertyName" ||
			(l &&
				((o = O.parent) === null || o === void 0 ? void 0 : o.name) == "Block")
		)
			return { from: O.from, options: m(), validFor: n };
		if (O.name == "ValueName") return { from: O.from, options: b, validFor: n };
		if (O.name == "PseudoClassName")
			return { from: O.from, options: h, validFor: n };
		if (O.name == "VariableName" || ((e.explicit || l) && Oe(O, r.doc)))
			return {
				from: O.name == "VariableName" ? O.from : a,
				options: W(r.doc, S(r).topNode),
				validFor: ae
			};
		if (O.name == "TagName") {
			for (let { parent: d } = O; d; d = d.parent)
				if (d.name == "Block")
					return { from: O.from, options: m(), validFor: n };
			return { from: O.from, options: ee, validFor: n };
		}
		if (!e.explicit) return null;
		let i = O.resolve(a),
			s = i.childBefore(a);
		return s && s.name == ":" && i.name == "PseudoClassSelector"
			? { from: a, options: h, validFor: n }
			: (s && s.name == ":" && i.name == "Declaration") || i.name == "ArgList"
			? { from: a, options: b, validFor: n }
			: i.name == "Block"
			? { from: a, options: m(), validFor: n }
			: null;
	},
	P = w.define({
		name: "css",
		parser: M.configure({
			props: [z.add({ Declaration: x() }), R.add({ Block: U })]
		}),
		languageData: {
...
function ue() {
	return new _(P, P.data.of({ autocomplete: oe }));
}
export { ue as css, oe as cssCompletionSource, P as cssLanguage };
//# sourceMappingURL=index-3d4c29df.js.map
