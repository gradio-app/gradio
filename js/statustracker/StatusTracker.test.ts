...
describe("StatusTracker: timer text color", () => {
	// The timer text declares its own color instead of inheriting it, so that whatever
	// the app container hands down cannot make it unreadable (#10940).
	const inherited_color = "rgb(255, 0, 0)";
	const body_text_color = "rgb(0, 128, 0)";
	...
	function setup(variant: "default" | "center"): HTMLElement {
		target = document.createElement("div");
		target.style.color = inherited_color;
		target.style.setProperty("--body-text-color", body_text_color);
		document.body.appendChild(target);

		component = mount(StatusTracker, {
			target,
			props: { ...base_props, variant }
		});

		return target.querySelector(".progress-text") as HTMLElement;
	}
	...
	test("timer text does not inherit the container text color", async () => {
		const progress_text = setup("default");
		await tick();

		expect(progress_text).toHaveClass("meta-text");
		expect(getComputedStyle(progress_text).color).toBe(body_text_color);
	});

	test("centered timer text does not inherit the container text color", async () => {
		const progress_text = setup("center");
		await tick();

		expect(progress_text).toHaveClass("meta-text-center");
		expect(getComputedStyle(progress_text).color).toBe(body_text_color);
	});
});
