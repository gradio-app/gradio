...
class ImageSlider(Component):
    ...
    def _process_image(self, image):
        if isinstance(image, str):
            im = processing_utils.decode_base64_to_image(image)
        else:
            im = _Image.open(self.make_temp_copy_if_needed(image["name"]))

        return self._format_image(im)

    def preprocess(
        self, x: tuple[str, str]
    ) -> tuple[
        np.ndarray | _Image.Image | str | None, np.ndarray | _Image.Image | str | None
    ]:
        """
        Parameters:
        ...
        return self._process_image(x[0]), self._process_image(x[1])

    def post_process_image(self, y):
        if y is None:
            return None
    ...
    def postprocess(
        ...
        return self.post_process_image(y[0]), self.post_process_image(y[1])
