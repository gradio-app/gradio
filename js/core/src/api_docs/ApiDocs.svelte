...
${info?.named_endpoints["/" + d.api_name]?.parameters?.map((p: any) => {
  ...
  const type =
    `${p.python_type.type}${p.parameter_has_default && p.parameter_default === null ? ' | None' : ''}`;
...
${info?.named_endpoints["/" + d.api_name]?.returns?.map((r: any, i: number) => {
...
${info?.named_endpoints["/" + d.api_name]?.parameters?.map((p: any) => {
  ...
  const defaultValue = !required ? `Default: ${represent_value(p.parameter_default, p.python_type.type, "py")}` : "Required";
  const type =
    `${js_info.named_endpoints["/" + d.api_name]?.parameters.find((_p: any) => _p.parameter_name === p.parameter_name)?.type || "any"}`;
