<script lang="ts">
	...
	function wait_then_trigger_api_call(
		...
		// TODO: fix this with scheduled updates
	...
	async function get_component_value_or_event_data(
		...
		return await get_data(component_id);
	}
	...
	// 				queue_position: null,
	// 			});
	// 			// set_status($loading_status); TODO: fix loading status store
	// 			return;
	// 		}
	...
	// 					// handle_data("set", validation_error_data);
	// 					loading_status.update({
	// 						status: "complete",
	...
	// 		app_tree.update_state(comp.id, { shared: { target } });
	// 	});
	...
	// 	// const deps = $targets[id]?.["change"]; // TODO: targets is not a store
