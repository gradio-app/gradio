...
demo_list = ["hello_world", "hello_world_2", "hello_world_3", "sepia_filter", "sales_projections",
         "calculator", "calculator_live", "blocks_hello", "blocks_flipper", "blocks_gpt", "blocks_speech_text_length",
         "blocks_essay"]
demos = []
...
def build(output_dir, jinja_env):
    ...
    template = jinja_env.get_template("playground/template.html")
    output = template.render(demos=demos)
    output_folder = os.path.join(output_dir, "playground")
