...
def nginx_endpoint(docker_ip, docker_services):
...
@pytest.fixture(scope="session")
def app_endpoint(docker_ip, docker_services):
...
def test_endpoint_status(app_endpoint, nginx_endpoint):
    for endpoint in [app_endpoint, nginx_endpoint]:
...
def test_api_response(app_endpoint, nginx_endpoint):
    for endpoint in [app_endpoint, nginx_endpoint]:
