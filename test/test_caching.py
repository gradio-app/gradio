...
class TestCacheDecorator:
    ...
    def test_runtime_wrapper_reuses_async_function(self):
        call_count = 0

        async def add_async(x, y=0):
            nonlocal call_count
            call_count += 1
            return x + y

        assert asyncio.run(cache(add_async)(1, y=2)) == 3
        assert asyncio.run(cache(add_async)(1, y=2)) == 3
        assert call_count == 1

    def test_runtime_wrapper_reuses_sync_generator(self):
        call_count = 0
