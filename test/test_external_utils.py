import pytest
from gradio.external_utils import create_endpoint_fn, component_from_parameter_schema, resolve_schema_ref, component_from_request_body_schema
import gradio as gr
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"name": "category", "schema": {"type": "object"}}, gr.JSON),
    ],
)
...
@pytest.mark.parametrize(
    "request_body,spec,expected_type",
    [
        (
...
def test_component_from_request_body_schema(request_body, spec, expected_type):
    comp = component_from_request_body_schema(request_body, spec)
    assert isinstance(comp, expected_type)
...
def test_create_endpoint_fn_docstring():
    operation = {
        "parameters": [
            {"name": "petId", "in": "path", "description": "ID of pet", "schema": {"type": "integer"}},
        ],
