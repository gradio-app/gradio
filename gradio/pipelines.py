...
def load_from_pipeline(pipeline: pipelines.base.Pipeline) -> dict:
    ...
    if "transformers.pipelines" in str(type(pipeline)):
    ...
    elif "diffusers.pipelines" in str(type(pipeline)):
        try:
            import diffusers
        ...
        raise ValueError(
            "pipeline must be a transformers.pipeline of diffusers.pipeline"
        )
    ...
    interface_info["title"] = (
        ...
        if "diffusers.pipelines" not in str(type(pipeline))
