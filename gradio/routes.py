...
async def process_video_with_ffmpeg(input_path, output_path, params):
    ...
    try:
        ...
        if (
            params["remove_segment"]["start"] is not None
            and params["remove_segment"]["end"] is not None
        ):
            start = params["remove_segment"]["start"]
            end = params["remove_segment"]["end"]

            if start < end:  # Only process if segment is valid
                temp_output = input_path.replace(".webm", "_temp.webm")

                # Create FFmpeg command to remove segment
                cmd = [
                    "ffmpeg",
                    "-y",
                    "-i",
                    current_input,
                    "-filter_complex",
                    f"[0:v]trim=start=0:end={start},setpts=PTS-STARTPTS[v1];"
                ...
                if process.returncode != 0:
                    ...
                    # If segment removal fails, continue with original
                else:
                    current_input = temp_output

        # Apply zoom effects if specified
        if params["zoom_effects"]:
            # Sort effects by start time
            zoom_effects = sorted(params["zoom_effects"], key=lambda x: x["startTime"])
            ...
            if probe_process.returncode != 0:
                print(f"FFprobe error: {probe_stderr.decode()}")
                # If probing fails, skip zoom effects
            else:
                video_info = json.loads(probe_stdout.decode())
                width = video_info["streams"][0]["width"]
                height = video_info["streams"][0]["height"]

                # Build filter complex for zoom effects
                # This is a simplified version - would need to be expanded for multiple effects
                if len(zoom_effects) > 0:
                    effect = zoom_effects[0]
                    start_time = effect["startTime"]
                    end_time = effect["endTime"]
                    start_zoom = effect.get("startZoom", 1.0)
                    end_zoom = effect.get("endZoom", 1.3)
                    center_x = effect.get("centerX", 0.5)
                    center_y = effect.get("centerY", 0.5)

                    temp_output = current_input.replace(".webm", "_zoom.webm")
                    ...
                    cmd = [
                        ...
                        current_input,
                        "-filter_complex",
                        f"[0:v]zoompan=z='min({start_zoom}+({end_zoom}-{start_zoom})*(n/(fps*({end_time - start_time}))),10)':"
                        f"x='iw/2-(iw/zoom/2)+{(center_x - 0.5) * width}':"
                        f"y='ih/2-(ih/zoom/2)+{(center_y - 0.5) * height}':"
                        f"d={math.ceil(30 * (end_time - start_time))}:fps=30[v]",
                        "-map",
                        "[v]",
                        "-map",
                        "0:a",
                    ...
                    if process.returncode != 0:
                        print(f"FFmpeg zoom error: {stderr.decode()}")
                    else:
                        current_input = temp_output
        ...
        if process.returncode != 0:
            ...
            # If watermark fails, use the current processed version
...
class App(FastAPI):
    ...
    def create_app(
        ...
        async def process_recording(
            ...
            try:
                if remove_segment_start and remove_segment_end:
                    start = float(remove_segment_start)
                    end = float(remove_segment_end)
                    ...
                    if start < end:
                        ...
                        temp_files.extend(
                            [segment_output, before_segment, after_segment]
                        )
                        ...
                        if (
                            os.path.exists(concat_file)
                            and os.path.getsize(concat_file) > 0
                        ):
                            cmd_concat = [
                                "ffmpeg",
                                "-y",
                ...
                if zoom_effects:
                    try:
                        zoom_effects_data = json.loads(zoom_effects)
                        ...
                        if zoom_effects_data:
                            for i, effect in enumerate(zoom_effects_data):
                                try:
                                    ...
                                    zoom_output = tempfile.mktemp(
                                        suffix=f"_zoom_{i}.mp4"
                                    )
                                    before_segment = tempfile.mktemp(
                                        suffix=f"_before_{i}.mp4"
                                    )
                                    zoom_segment = tempfile.mktemp(
                                        suffix=f"_zoom_segment_{i}.mp4"
                                    )
                                    after_segment = tempfile.mktemp(
                                        suffix=f"_after_{i}.mp4"
                                    )
                                    concat_file = tempfile.mktemp(
                                        suffix=f"_concat_{i}.txt"
                                    )
                                    ...
                                    if start_time > 0:
                                        ...
                                        if process.returncode != 0:
                                            print(
                                                f"FFmpeg before segment error: {stderr.decode()}"
                                            )
                                    ...
                                    if process.returncode != 0:
                                        print(
                                            f"FFmpeg extract zoom segment error: {stderr.decode()}"
                                        )
                                        continue

                                    zoomed_segment = tempfile.mktemp(
                                        suffix=f"_zoomed_{i}.mp4"
                                    )
                                    ...
                                    if process.returncode != 0:
                                        print(
                                            f"FFmpeg zoom effect error: {stderr.decode()}"
                                        )
                                    ...
                                    if process.returncode != 0:
                                        print(
                                            f"FFmpeg after segment error: {stderr.decode()}"
                                        )
                                    ...
                                    print(
                                        f"Error processing zoom effect {i + 1}: {str(e)}"
                                    )
                                    traceback.print_exc()
                    except Exception as e:
