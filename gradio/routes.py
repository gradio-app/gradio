    ...
    def create_app(
        ...
        def openapi_schema(request: fastapi.Request):
            ...
            for endpoint_path, endpoint_info in info.get("named_endpoints", {}).items():  # type: ignore
                if not endpoint_info.get("show_in_view_api", True):
                    continue
                path_item = {
                    "post": {
...
def mount_gradio_app(
    ...
    *,
    ...
    Parameters:
        ...
        show_view_api: If False, hides the "Use via API" button on the Gradio interface.
    ...
    if show_view_api is not None:
        blocks.show_view_api = show_view_api
