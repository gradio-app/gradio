...
class Knob(
    ...
    """
    Creates a knob that ranges from `minimum` to `maximum` with a step size of `step`.
    Preprocessing: passes knob value as a {float} into the function.
    Postprocessing: expects an {int} or {float} returned from function and sets knob value to it as long as it is within range.
    Examples-format: A {float} or {int} representing the knob's value.

    Demos: knob_component
    Guides: TBD
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Parameters:
            minimum: minimum value for knob.
            maximum: maximum value for knob.
            value: default value. If callable, the function will be called whenever the app loads to set the initial value of the component. Ignored if randomized=True.
            step: increment between knob values.
            label: component name in interface.
            info: additional component description.
            every: If `value` is a callable, run the function 'every' number of seconds while the client connection is open. Has no effect otherwise. Queue must be enabled. The event can be accessed (e.g. to cancel it) via this component's .load_event attribute.
            show_label: if True, will display label.
            ...
            elem_id: An optional string that is assigned as the id of this component in the HTML DOM. Can be used for targeting CSS styles.
            elem_classes: An optional list of strings that are assigned as the classes of this component in the HTML DOM. Can be used for targeting CSS styles.
            randomize: If True, the value of the knob when the app loads is taken uniformly at random from the range given by the minimum and maximum.
        """
        self.minimum = minimum
        self.maximum = maximum
        if step is None:
    ...
    def set_interpret_parameters(self, steps: int = 8) -> Knob:
        """
        Calculates interpretation scores of numeric values ranging between the minimum and maximum values of the knob.
        Parameters:
            steps: Number of neighboring values to measure between the minimum and maximum values of the knob range.
        """
        self.interpretation_steps = steps
        return self
