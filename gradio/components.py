...
class VideoWithCaption(
    ...
    """
    Add the caption feature to the video component.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Parameters:
            value: List of video and caption path or URL for the default value that Video component is going to take. If callable, the function will be called whenever the app loads to set the initial value of the component.
            format: Format of video format to be returned by component, such as 'avi' or 'mp4'. Use 'mp4' to ensure browser playability. If set to None, video will keep uploaded format.
            label: component name in interface.
            every: If `value` is a callable, run the function 'every' number of seconds while the client connection is open. Has no effect otherwise. Queue must be enabled. The event can be accessed (e.g. to cancel it) via this component's .load_event attribute.
            show_label: if True, will display label.
    ...
    def postprocess(self, y: List[str] | None) -> List[Dict[str, str]] | None:
        """
        Processes a video to ensure that it is in the correct format before
        Parameters:
            y: List of video and caption path or URL to the video file
        Returns:
            a dictionary with the following keys: 'name' (containing the file path
            to a temporary copy of the video), 'data' (None), and 'is_file` (True).
        """
        ...
        video_path, caption_path = y

        returned_format = video_path.split(".")[-1].lower()
        ...
        # For cases where the video is a URL and does not need to be converted to another format, we can just return the URL
        if utils.validate_url(video_path) and not (conversion_needed):
            return [
                {"name": video_path, "data": None, "is_file": True},
                {"name": caption_path, "data": None, "is_file": True},
            ]

        # For cases where the video needs to be converted to another format
        if utils.validate_url(video_path):
            video_path = self.download_temp_copy_if_needed(video_path)
        if (
            processing_utils.ffmpeg_installed()
            and not processing_utils.video_is_playable(video_path)
        ):
            warnings.warn(
                "Video does not have browser-compatible container or codec. Converting to mp4"
            )
            video_path = processing_utils.convert_video_to_playable_mp4(video_path)
        if self.format is not None and returned_format != self.format:
            output_file_name = video_path[0 : video_path.rindex(".") + 1] + self.format
            ff = FFmpeg(inputs={video_path: None}, outputs={output_file_name: None})
            ff.run()
            video_path = output_file_name

        video_path = self.make_temp_copy_if_needed(video_path)
        caption_path = self.make_temp_copy_if_needed(caption_path)
        return [
            {"name": video_path, "data": None, "is_file": True},
            {"name": caption_path, "data": None, "is_file": True},
        ]
