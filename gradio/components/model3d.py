    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            camera_position: camera position of scene, provided as a tuple of `(alpha, beta, radius)`. Each value is optional. If provided, `alpha` and `beta` should be in degrees reflecting the angular position along the longitudinal and latitudinal axes, respectively. Radius corresponds to the distance from the center of the object to the camera. As the user moves the camera, this value is updated to reflect the current camera position, so a function whose parameter is annotated with `gr.Model3D` reads the live position as a list of three floats. For .splat files, and for .ply files holding a Gaussian splat, this parameter is ignored and is not updated, as those are rendered by a viewer that does not expose the camera.
