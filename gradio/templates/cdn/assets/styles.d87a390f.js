const create_classes = (styles, prefix = "") => {
  ...
  if (target_styles.hasOwnProperty("rounded")) {
    if (!Array.isArray(target_styles.rounded)) {
      target_styles.rounded = !!target_styles.rounded ? [true, true, true, true] : [false, false, false, false];
    }
    let rounded_map = ["tl", "tr", "br", "bl"];
    target_styles.rounded.forEach((rounded, i) => {
