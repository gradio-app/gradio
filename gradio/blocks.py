    ...
    def launch(
        ...
        *,
        ...
        Parameters:
            ...
            run_history: If True, users can review and reload calls from the run history page at /gradio_api/runs. Runs are saved privately in the browser by default; from that page, a user can instead connect a Hugging Face bucket and save future runs there. Where the host platform keeps history for its users, it attaches the bucket and a credential scoped to it on each request, and a caller who has connected no bucket of their own is recorded there instead. Browser history is scoped to the logged-in user if the app uses `auth`. If False, nothing is recorded, the run history page is disabled, and any runs previously saved by this app are deleted from the browser. If None, will use the GRADIO_RUN_HISTORY environment variable or default to True.
