...
class Session:
    ...
    async def init(self):
        ...
        # task2 = asyncio.create_task(self.send())
        # await task2
        # await task1
    ...
    async def send(self):
        ...
        while True:
            ...
            if len(self.sendCh) > 0:
                ...
                print(">>>>>>>>", ready["Hdr"].h, ready.get("Body"))
    ...
    async def recvLoop(self):
        ...
        while True:
            ...
            print("recv_header->flags:%d,msgType:%d" % (hdr.flags(), hdr.msgType()))
    ...
    async def handleStreamMessage(self, hdr: header.Header):
        sid = hdr.streamID()
        flags = hdr.flags()
        print("handleStreamMessage->sid:%s,flags:%d" % (sid, flags))
