...
/**
 * Create a fetch() function that proxies HTTP requests to the worker,
 * which also falls back to the original fetch() for external resource requests.
 */
export function makeWasmFetch(workerProxy: WorkerProxy): typeof fetch {
	async function overriddenFetch(
