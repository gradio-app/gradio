...
class HighlightedTextOutput extends React.Component {
  ...
  render() {
    ...
    if (first_score === null || typeof first_score === "string") {
      ...
      return <div className="output_highlightedtext">
        <div className="category_legend">
          {Object.entries(this.color_map).map(([category, color]) => {
            return (<div className="category-label">
        ...
        <div className="output_text">
          {this.props.value.map((span) => {
            ...
            return (<span title={category} style={{ "background-color": color }}>{span[0]}</span>)
          })
          }
        </div>
      </div>
      ...
      return <div className="output_highlightedtext">
        ...
        <div className="output_text">
          {this.props.value.map((text, value) => {
            ...
            return <span title={value} style={{"background-color": `rgba{${color}}`}}>{text}</span>
          })}
        </div>
      </div>
    }
...
class HighlightedTextOutputExample extends ComponentExample {
  render() {
    ...
    return <div className="output_highlighted_text_example">{output_string}</div>
  }
}
